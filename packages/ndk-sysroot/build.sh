...
termux_step_make_install() {
    ...
    # librt and libpthread are built into libc on android, so setup them as symlinks
    # to libc for compatibility with programs that users try to build:
    for lib in librt.so libpthread.so; do
        echo 'INPUT(-lc -landroid-support)' > $TERMUX_PKG_MASSAGEDIR/$TERMUX_PREFIX/lib/$lib
    done
    unset lib

    for dir in toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/*; do
